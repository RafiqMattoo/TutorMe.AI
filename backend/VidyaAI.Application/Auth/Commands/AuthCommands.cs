...
// ── FORGOT PASSWORD / RESET COMMANDS ───────────────────────────
    ...
    : IRequestHandler<ForgotPasswordCommand>
{
    ...
    {
        var user = await db.Users.FirstOrDefaultAsync(u => u.Email == req.Email && !u.IsDeleted, ct);

        // Always succeed to avoid account enumeration; only act when user exists.
        if (user is null) return;

        var token = Guid.NewGuid().ToString("N");

        var oneTime = new VidyaAI.Domain.Entities.OneTimeToken
        {
            UserId = user.Id,
            Token = token,
            Purpose = VidyaAI.Domain.Entities.OneTimeTokenPurpose.PasswordReset,
            ExpiresAt = DateTime.UtcNow.AddHours(1)
        };

        await db.OneTimeTokens.AddAsync(oneTime, ct);
        await db.SaveChangesAsync(ct);

        var (subject, html) = VidyaAI.Application.Common.EmailTemplates.PasswordReset(user.FirstName + " " + user.LastName, token);
...
public sealed class ResetPasswordCommandValidator : AbstractValidator<ResetPasswordCommand>
{
    public ResetPasswordCommandValidator()
    {
        ...
        RuleFor(x => x.NewPassword).NotEmpty().MinimumLength(6).WithMessage("Password required.");
    ...
    : IRequestHandler<ResetPasswordCommand>
{
    public async Task Handle(ResetPasswordCommand req, CancellationToken ct)
    {
        var oneTime = await db.OneTimeTokens
            .Include(t => t.User)
            .FirstOrDefaultAsync(t => t.Token == req.Token && t.Purpose == VidyaAI.Domain.Entities.OneTimeTokenPurpose.PasswordReset, ct)
            ?? throw new InvalidOperationException("Invalid or expired reset token.");

        if (oneTime.IsUsed || oneTime.ExpiresAt < DateTime.UtcNow) throw new InvalidOperationException("Invalid or expired reset token.");

        var user = oneTime.User;
        user.PasswordHash = BCrypt.Net.BCrypt.HashPassword(req.NewPassword);
        oneTime.IsUsed = true;
        oneTime.UsedAt = DateTime.UtcNow;
